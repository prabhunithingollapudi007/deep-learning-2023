...
class Conv(BaseLayer):
    ...
    def __init__(self, stride_shape, convolution_shape, num_kernels):
        # Setting the trainable parameter to True
        super().__init__()
        self.trainable = True
    ...
    def pad_input(self, input_tensor, p1, p2):
    ...
    def calculate_output_dimensions(self, padded_image):
    ...
    def perform_convolution(self, input_tensor, output_tensor, h_cnn, v_cnn):
    ...
    def forward(self, input_tensor):
        ...
        padded_image = self.pad_input(input_tensor, p1, p2)
        input_tensor = padded_image
        self.padded = padded_image.copy()

        h_cnn, v_cnn = self.calculate_output_dimensions(padded_image)
        output_tensor = np.zeros((input_tensor.shape[0], self.num_kernels, int(h_cnn), int(v_cnn)))
        self.output_shape = output_tensor.shape

        output_tensor = self.perform_convolution(input_tensor, output_tensor, h_cnn, v_cnn)
    ...
    def backward(self, error_tensor):
        '''
        Implementation of the backward pass of the convolution layer
        '''
